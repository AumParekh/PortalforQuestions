...
U6_SOURCE = (
    ...
    "which reproduces the notes' 55.5% / 37.0% / ~8% benchmark split (l.295-297, l.304)."
)
...
SPECS = [
    ...
    mk(id='fr-IM-5-01', reading_id='IM-5', source_block='im5.d.exbox.2', source_file=IM5, source_line=545,
       ...
       explanation='At the optimum the ratios E_i / beta_i (equivalently E_i / MVaR_i) are equal across assets (IM-5 l.523-526). Raising the CAD\'s E_i breaks the equality in the CAD\'s favour, so the rule "add to the position with the highest excess expected return to marginal VaR" (l.533) moves weight into CAD (90.2% -> 92.0%) until the ratios re-equalise. Because the original optimum already sits to the right of the minimum-risk mix (13.98 vs 13.85 in the notes\' units, l.559 / l.505), more CAD means a little more risk: the point climbs up and slightly right.',
       notes_checks=[
       ...
       citations=[dict(line=546, note='CAD 8%, EUR 5% excess returns', literal='8\\% for CAD'),
                  dict(line=526, note='optimality: excess return / MVaR equal across assets', literal='MVaR'),
    ...
    mk(id='fr-IM-5-02', reading_id='IM-5', source_block='im5.d.fmlbox.1', source_file=IM5, source_line=523,
       ...
       citations=[dict(line=526, note='optimality condition', literal='MVaR'),
    ...
    mk(id='fr-IM-5-03', reading_id='IM-5', source_block='im5.d.prose_para.1', source_file=IM5, source_line=474,
       ...
       notes_checks=[
           ...
           dict(quantity='sigma', at='from', multiplier=3.0, expected=13.85, tol=0.005, line=505, literal='13.85'),
       ],
    ...
    mk(id='fr-IM-5-04', reading_id='IM-5', source_block='im5.d.prose_para.1', source_file=IM5, source_line=478,
    ...
    mk(id='fr-IM-5-05', reading_id='IM-5', source_block='im5.b.prose_para.1', source_file=IM5, source_line=329,
       ...
       options=[
           ...
           dict(label='Stays: correlation does not change individual volatilities', move='stays', correct=False,
                why='Individual risks are unchanged, but portfolio risk depends on the cross term 2 w1 w2 rho sigma1 sigma2 (l.89).'),
       ],
       explanation='Correlation sets where portfolio risk sits (IM-5 l.329): diversified VaR is below undiversified VaR whenever correlation is below 1 (l.92-93), and the cross term 2 w1 w2 rho sigma1 sigma2 (l.89) turns negative when rho < 0. The minimum-variance mix shifts toward more EUR (22.6%) to exploit the hedge; per-dollar risk falls from 4.62% to 3.73% and, since EUR earns less than CAD, expected return slips from 7.56% to 7.32%.',
       notes_checks=[],
       citations=[dict(line=329, note='correlation sets portfolio risk between bounds', literal='Correlation sets'),
                  dict(line=89, note='two-asset variance formula with the rho cross term', literal='2w_1w_2\\rho_{12}'),
    ...
    mk(id='fr-IM-5-07', reading_id='IM-5', source_block='im5.e.defbox.1', source_file=IM5, source_line=628,
       ...
       citations=[dict(line=629, note='risk management -> global minimum', literal='Risk management'),
                  dict(line=636, note='marginal VaR is the instrument in both cases', literal='instrument in both cases')],
       cal_slope=None),
    ...
    mk(id='fr-IM-6-03', reading_id='IM-6', source_block='im6.h.tikzpicture.1', source_file=IM6, source_line=319,
       ...
       explanation='Relative risk budgets follow relative IRs (IM-6 figure caption l.353-357: IR ratio 1.50 gives risk-budget ratio 1.50). When both IRs are 0.60 the ratio is 1.00 and the mix is 50/50. For two independent managers with equal TEV, 50/50 is also the minimum-TEV mix, so the point moves up (3.12% -> 3.60%) and to the left (4.33% -> 4.24%). The combined IR rises to 0.85 = 0.6 x sqrt(2).',
       notes_checks=[dict(quantity='w[0]', at='from', expected=0.60, tol=1e-9, line=280, literal='0.60')],
       citations=[dict(line=354, note='IR ratio 1.50 = risk-budget ratio 1.50', literal='a ratio of $1.50$'),
                  dict(line=285, note='risk budgets proportional to IR', literal='proportional to information ratios')],
       cal_slope='steeper'),

    # ------------------------------------------------------------------ IM-4 (active risk aversion)
    mk(id='fr-IM-4-01', reading_id='IM-4', source_block='im4.e.fmlbox.1', source_file=IM4, source_line=156,
       ...
       notes_checks=[dict(quantity='sigma', at='from', expected=4.0, tol=0.001, line=279, literal='TEV of 4', source_file=IM6),
                     dict(quantity='yw[0]', at='from', expected=0.555, tol=0.001, line=304, literal='0.555', source_file=IM6),
                     dict(quantity='yw[1]', at='from', expected=0.370, tol=0.001, line=304, literal='0.370', source_file=IM6),
                     dict(quantity='mu', at='from', expected=2.9, tol=0.02, line=304, literal='2.9', source_file=IM6)],
       citations=[dict(line=156, note='lambda_A = IR / (2 sigma_A)', literal='\\lambda_A = \\frac{\\text{IR}}{2\\,\\sigma_A}'),
                  dict(line=158, note='percentages, not decimals', literal='Percentages, not decimals'),
                  dict(line=164, note='worked example lambda_A = 0.05', literal='0.05')],
       cal_slope='same',
    ...
    mk(id='fr-IM-4-02', reading_id='IM-4', source_block='im4.i.trapbox.1', source_file=IM4, source_line=331,
       title='Same risk aversion, better information ratio',
       assets=U6_ASSETS, corr=U6_CORR, rf=0.0, point='complete', A=U6_A,
       slider=dict(param='mu[0]', label='Manager 1 expected active return (%) [IR = value / 6]', frm=3.6, to=4.0, step=0.05),
       question='Active risk aversion is fixed. Manager 1\'s IR improves from 0.60 to about 0.67 (expected active return 3.6% -> 4.0%, TEV 6%). Where does the fund\'s chosen point (manager mix + benchmark) go?',
       options=[
           dict(label='Up and to the right, onto a steeper line: the fund takes more active risk', move='up-right', correct=True,
                why='Rearranging lambda_A = IR / (2 sigma_A) gives sigma_A = IR / (2 lambda_A): with the aversion fixed, a higher IR buys more active risk, and more active return with it.'),
           dict(label='Down-left along the old line: the better manager lets the fund hit its return with less risk', move='down-left', correct=False,
                why='That would be a TARGET-return investor. Here the risk aversion is fixed, and the optimal active risk rises in proportion to the IR.'),
           dict(label='Straight up: same 4% TEV, more active return', move='up', correct=False,
                why='A fixed 4% TEV is a constraint (IM-6), not a risk aversion. With lambda_A fixed, the chosen TEV moves with the IR: 4.00% -> 4.31%.'),
           dict(label='Stays: risk aversion, not skill, sets the point', move='stays', correct=False,
                why='The point depends on both: sigma_A = IR / (2 lambda_A).'),
       ],
       explanation='IM-4 links the three quantities: lambda_A = IR / (2 sigma_A) (l.156; trap summary l.331), so the desired active risk is sigma_A = IR / (2 lambda_A). In the game\'s convention A = 2 lambda_A, so the complete point sits at sd = IR_T / A. Start as in fr-IM-4-01 (IM-6 managers, TEV 4.00%, IR 0.72). Raising Manager 1\'s IR to 0.667 lifts the best mix\'s IR to 0.777 and, at the same A, the chosen active risk to 4.31% and expected active return from 2.88% to 3.35%. The extra risk budget goes to Manager 1 (55.5% -> 61.6% of the fund) while Manager 2 keeps 37.0%; the benchmark holding shrinks from 7.6% to 1.4%.',
       notes_checks=[dict(quantity='sigma', at='from', expected=4.0, tol=0.001, line=279, literal='TEV of 4', source_file=IM6),
                     dict(quantity='sharpe_T', at='from', expected=0.72, tol=0.005, line=307, literal='0.72', source_file=IM6)],
       citations=[dict(line=156, note='lambda_A = IR / (2 sigma_A)', literal='\\lambda_A = \\frac{\\text{IR}}{2\\,\\sigma_A}'),
                  dict(line=331, note='trap summary: lambda_A = IR/(2 sigma_A)', literal='Formula, IM-4 e.'),
                  dict(line=81, note='active managers target a level of active risk consistent with the aversion', literal='Active risk aversion')],
       cal_slope='steeper',
       extra_sources=[dict(reading_id='IM-6', source_file=IM6, lines=[279, 280, 304, 307],
                           note='manager inputs and the 4% TEV starting point are the IM-6 worked example')]),
    ...
    mk(id='fr-IM-1-02', reading_id='IM-1', source_block='im1.c.fmlbox.1', source_file=IM1, source_line=137,
       ...
       notes_checks=[dict(quantity='y', at='from', expected=1.0, tol=1e-3, line=129, literal='holds the market')],
       citations=[dict(line=137, note='E(Rm) - Rf = gamma-bar sigma_m^2', literal='\\bar{\\gamma}'),
...
def build():
    ...
    return {
        ...
        'omitted': [
            dict(reading_id='IM-17', reason='States that optimal holdings of illiquid assets are lower (l.210-211) and that unsmoothing raises risk estimates but not means (l.105), but gives no volatilities or correlations; any frontier would need invented inputs.'),
